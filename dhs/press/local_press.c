...
*	int	press_null_out	: Counts characters.
...
*	99/10/12 NRH  : Added buffering of input when reading from files
*			for performance reasons.
*	00/08/10 SJG  :	Added press_null_out function.
*
****  C A N A D I A N   A S T R O N O M Y   D A T A   C E N T R E  *****
************************************************************************
-*/
...
)
{
    ...
    ptr1 = buffer;
    bytes_to_get = length;
    bytes_in_buffer = local_press.lp_in_buf_size - local_press.lp_in_buf_pos;
    for ( ptr2 = local_press.lp_in_buf + local_press.lp_in_buf_pos; 
	    bytes_to_get > 0 &&  bytes_in_buffer > 0; 
	    bytes_to_get --, bytes_in_buffer --, local_press.lp_in_buf_pos ++,
	    ptr1++, ptr2++ )
    {
	*ptr1 = *ptr2;
    }
    ...
    if ( bytes_to_get > PR_BUFFER_SIZE )
    {
	len = read( local_press.lp_infile, ptr1, bytes_to_get );
	bytes_to_get -= len;
    }
    else
    {
	local_press.lp_in_buf_size = read( local_press.lp_infile, 
		local_press.lp_in_buf, PR_BUFFER_SIZE );
	local_press.lp_in_buf_pos = 0;
	for ( ptr2 = local_press.lp_in_buf,
		bytes_in_buffer = local_press.lp_in_buf_size; 
		bytes_to_get > 0 && bytes_in_buffer > 0;
		bytes_to_get --, bytes_in_buffer --, 
		local_press.lp_in_buf_pos ++, ptr1 ++, ptr2 ++ )
	{
	    *ptr1 = *ptr2;
	}
    }

    if ( bytes_to_get == length )
    {
	return( PR_E_EOI );
    }
    else
    {
	return( length - bytes_to_get );
    }
}

/*+
...
************************************************************************
*
*   Function:	press_null_out
*
*   Purpose:
*	Counts the number of characters to copy.
*
*   Values Returned:
*	int	PR_SUCCESS	: Normal completion.
*
