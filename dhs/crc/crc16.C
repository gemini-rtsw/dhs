...
// Implementation of 16-bit CRC (cyclic redundancy check) class:
//     Polynomial:                 1021
//     Initial CRC register value: FFFF
//     Reflected input and output: No
//     Inverted final output:      No
//     CRC of string "123456789":  29B1

static __uint16 m_table16n[256] = {
...
void crc16::compute(const void* buffer, unsigned int count)
{
    const unsigned char* ptr = (const unsigned char *) buffer;
    while (count--) {
        m_crc = m_table16n[((m_crc >> 8) ^ *ptr++)] ^ (m_crc << 8);
    }
}

void crc16::compute(unsigned char value)
{
    m_crc = m_table16n[((m_crc >> 8) ^ value)] ^ (m_crc << 8);
}
