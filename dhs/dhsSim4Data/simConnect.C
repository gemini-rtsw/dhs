static char rcsid[] = "$Id: simConnect.C,v 1.1.1.1 2002-11-24 20:30:11 brighton Exp $";
//
//***********************************************************************
...
//
// Revision 1.2  1998/04/20 20:53:11  nhill
// Updated to use the latest version of the cDhsStatus class.
//
// Revision 1.1  1997/10/03 23:07:59  cockayne
// Initial revision
//
//INDENT-ON*
//
...
)
{
    ...
    char	*id;		// Identity of the connector.
    cSimStatus
		sStatus;	// Status. Some reason for the 's', I'm sure. 


    if ( status() == DHS_S_CONNECT )
    {
	//
	//  Connection attempt. Just print out info about the connector. 
	//

	checkDhs( dhsConnectInfo( connect(), &ip, &id, &dhsStatus ),
	    dhsStatus, sStatus, VOID );

	sStatus.S_CONNECT_REQ( sStatus, id, ip );
    }
    else
    {
	//
	//  Disconnection attempt. Just print out info about the connector. 
	//

	checkDhs( dhsConnectInfo( connect(), &ip, &id, &dhsStatus ),
	    dhsStatus, sStatus, VOID );

	sStatus.S_DISCONNECT_REQ( sStatus, id, ip );
    }
}
