...
*	98/05/15 SEC  :	For some unknown reason, va_start() was only
*			being called with one argument. Fixed.
*	99/10/12 NRH  : Added initialization of the input buffers when reading
*			from files. 
*
****  C A N A D I A N   A S T R O N O M Y   D A T A   C E N T R E  *****
************************************************************************
...
{
    local_press.lp_in_buf = local_press.lp_in_buffer;
    local_press.lp_in_buf_size = 0;
...
)
{
	...
	*osize = LG( buf );
	...
	local_press.lp_out_buf_size = 0;
	local_press.lp_out_buf_pos = 0;

	PR_CHECK( unpress( press_file_in, press_null_out, type ) );
