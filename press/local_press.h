...
*	99/10/12 NRH  : Added lp_in_buffer to the local structure to
*			be used when buffering input from files.
...
typedef struct local_press 
{
    ...
    char	lp_in_buffer[PR_BUFFER_SIZE];
				/* Temporary buffer for when reading	*/
				/* from a file.				*/
...
extern	int	press_null_out( byte *, int );
